...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Image size
    gs = max(int(model.detect_block.stride.max()), 32)  # grid size (max stride)
    ...
    # Trainloader
    train_loader, dataset = create_rgbtdataloader(train_path,
                                              imgsz,
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loader = create_rgbtdataloader(val_path,
                                       imgsz,
        ...
        if not resume:
            if not opt.noautoanchor:
                check_anchors(dataset, model=model.detect_block, thr=hyp['anchor_t'], imgsz=imgsz)  # run AutoAnchor
    ...
    # Model attributes
    nl = de_parallel(model).detect_block.nl  # number of detection layers (to scale hyps)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (img_rgb, img_t, targets, rgb_paths, t_paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            img_t = img_t.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            if opt.multi_scale:
                ...
                rgb_sf = sz / max(img_rgb.shape[2:])  # scale factor
                if rgb_sf != 1:
                    ns = [math.ceil(x * rgb_sf / gs) * gs for x in img_rgb.shape[2:]]  # new shape (stretched to gs-multiple)
                    img_rgb = nn.functional.interpolate(img_rgb, size=ns, mode='bilinear', align_corners=False)

                t_sf = sz / max(img_t.shape[2:])  # scale factor
                if t_sf != 1:
                    ns = [math.ceil(x * t_sf / gs) * gs for x in img_t.shape[2:]]  # new shape (stretched to gs-multiple)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default=ROOT / 'configs/hyp.scratch-low.yaml', help='hyperparameters path')
