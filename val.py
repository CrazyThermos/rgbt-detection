...
def run(
    ...
    if not training:
        ...
        dataloader = create_rgbtdataloader(data[task],
                                       imgsz,
                                       batch_size,
                                       stride,
                                       single_cls,
    ...
    for batch_i, (im_rgb, im_t, targets, rgb_paths, t_paths, shapes) in enumerate(pbar):
        ...
        im_t = im_t.half() if half else im_t.float()  # uint8 to fp16/32
        im_rgb /= 255  # 0 - 255 to 0.0 - 1.0
        im_t /= 255  # 0 - 255 to 0.0 - 1.0

        rgb_nb, _, rgb_height, rgb_width = im_rgb.shape  # batch size, channels, height, width
        ...
        # NMS
        targets[:, 2:] *= torch.tensor((t_width, t_height, t_width, t_height), device=device)  # to pixels
        lb = [targets[targets[:, 0] == i, 1:] for i in range(t_nb)] if save_hybrid else []  # for autolabelling
        # with dt[2]:
        ...
        for si, pred in enumerate(preds):
            ...
            if single_cls:
                pred[:, 5] = 0
            predn = pred.clone()
            scale_boxes(im_t[si].shape[1:], predn[:, :4], shape, shapes[si][1])  # native-space pred

            # Evaluate
            if nl:
                tbox = xywh2xyxy(labels[:, 1:5])  # target boxes
                scale_boxes(im_t[si].shape[1:], tbox, shape, shapes[si][1])  # native-space labels
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, None #t
