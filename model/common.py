...
import numpy as np
def attempt_load(model_name, weights, device, training=True):
...
class RGBTDetectMultiBackend(nn.Module):
    ...
    def __init__(self,model_name='rgbt_yolov5', weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        # Usage:
        #   PyTorch:              weights = *.pt
        ...
        jit = False
        engine = weights[0].split('.')[-1] == 'engine'
        onnx = False
        saved_model = False
        pb = False
        triton = False
        ...
        if pt:  # PyTorch
            ...
            if isinstance(weights, list):
                weights = weights[0]
            backendmodel = attempt_load(model_name = model_name, weights=weights, device=device, training=False)
            LOGGER.info(f'from factory get model:{model_name}')  # report
            
            # ckpt = torch.load(weights, map_location='cpu') 
            # csd = ckpt.get('ema').float().state_dict()
            # csd = intersect_dicts(csd, backendmodel.state_dict(), exclude=[])
            # backendmodel.load_state_dict(csd, strict=True)
            ...
            self.model = backendmodel.eval()  # explicitly assign for to(), cpu(), cuda(), half()
        elif engine:
