...
from model.fuseblock import fuse_block_conv1x1,fuse_block_AFF,fuse_block_CA,fuse_block_Mamba
...
class rgbt_yolov5_Mamba(nn.Module):
    def __init__(self, ch, gd=1.0, gw=1.0, last_ch=1024, nc=2, training=False) -> None:
        super().__init__()
        self.gd = gd
        self.gw = gw
        ...
        self.fuse_block1 = fuse_block_Mamba(ch=self.gw_div(last_ch//8), img_size=160, patch_size=8, d_model=4096)
        self.fuse_block2 = fuse_block_Mamba(ch=self.gw_div(last_ch//4), img_size=80,  patch_size=4, d_model=4096)
        self.fuse_block3 = fuse_block_Mamba(ch=self.gw_div(last_ch//2), img_size=40,  patch_size=2, d_model=4096)
        self.fuse_block4 = fuse_block_Mamba(ch=self.gw_div(last_ch), img_size=20,  patch_size=1, d_model=4096)
    ...
    def forward(self, rgb, t):
        ...
        rgb_f2 = self.rgb_block2(rgb_f1 + fuse_f1[0])
        t_f2 = self.t_block2(t_f1 + fuse_f1[1])
        fuse1 = rgb_f2 + t_f2
        fuse_f2 = self.fuse_block2(rgb_f2, t_f2)
        
        rgb_f3 = self.rgb_block3(rgb_f2 + fuse_f2[0])
        t_f3 = self.t_block3(t_f2 + fuse_f2[1])
        fuse2 = rgb_f3 + t_f3
        fuse_f3 = self.fuse_block3(rgb_f3, t_f3)
        
        rgb_f4 = self.rgb_block4(rgb_f3 + fuse_f3[0])
        t_f4 = self.t_block4(t_f3 + fuse_f3[1])
        rgb_f4 = self.rgb_sppf(rgb_f4)
        t_f4 = self.t_sppf(t_f4)
        fuse3 = rgb_f4 + t_f4
